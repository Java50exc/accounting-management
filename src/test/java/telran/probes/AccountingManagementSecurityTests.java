...
public class AccountingManagementSecurityTests {
	...
	void addAccount_admin_success() throws Exception {
		...
		mockMvc.perform(
				post(ACCOUNTS_PATH).contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(ACCOUNT_DTO)))
	...
	@Test
	@WithMockUser
	void updatePassword_admin_success() throws Exception {
		doNothing().when(accountService).updatePassword(anyString(), anyString());
		mockMvc.perform(put(ACCOUNTS_PATH).contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(PASSWORD_UPDATE_DTO))).andExpect(status().isOk());
	}
	...
	void updatePassword_unauthorized_401() throws Exception {
		doNothing().when(accountService).updatePassword(anyString(), anyString());
		mockMvc.perform(put(ACCOUNTS_PATH).contentType(MediaType.APPLICATION_JSON).content(mapper.writeValueAsString(PASSWORD_UPDATE_DTO))).andExpect(status().isUnauthorized());
	}
